    ...
    <div class="row g-4">
        <div class="col-sm-12 col-xl-6 form-control">
            <div class="bg-light rounded h-100 p-4 form-control">
                <h6 class="mb-4">Postni yangilash</h6>
                <form method="POST" action="{{ route('admin.posts.update',$post->id) }}" enctype="multipart/form-data">
                    ...
                    <select class="select form-control" name="category_id" id="" >
                        @foreach($categories as $item)
                            <option @if ($item->id = $post->category_id) selected @endif value="{{ $item->id }}">{{ $item->name_uz }}</option>
                        @endforeach
                    </select> <br>

                    <strong>Tag:</strong>
                    <select  class="select form-control" name="tag_id" id="example-multple-selected" name="tag_id[]" multiple>
                        @foreach($tags as $item)
                            <option
                            @foreach ($post->tags as $postTag )

                            @if ($postTag->id == $item->id) selected  @endif

                            @endforeach
                            value="{{ $item->id }}">{{ $item->tag_uz }}</option>
                        @endforeach
                    </select> <br>
